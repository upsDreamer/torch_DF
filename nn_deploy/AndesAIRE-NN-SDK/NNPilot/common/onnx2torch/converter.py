...
def switch_paddings(model):
    ...
    for name,module in model.named_modules():
        if isinstance(module, OnnxPadStatic):
            paddings_info = copy.deepcopy(module.pads)
            switch_layer = torch.nn.ZeroPad2d(paddings_info)
            set_layer(model, name, switch_layer)
    return  model
...
def convert(  # pylint: disable=too-many-locals, too-many-branches, too-many-statements
    ...
    for name, onnx_node in onnx_graph.nodes.items():
        ...
        for value_name in onnx_mapping.inputs:
            ...
            if value_type == ValueType.GRAPH_INPUT:
                ...
                if len(onnx_input_node.output_values) > 1 or 'Split' in onnx_input_node.unique_name:
    ...
    for node in torch_graph.nodes:
        if node.op=="call_module":
            if isinstance(getattr(torch_modules,node.target),node_converters.OnnxScatter1D):
                create_ouput_node.append(node)
            elif node.args==():
                print(node.name)
                node.op="placeholder"
    """
    Insert output after scattered1d
    """
    if len(create_ouput_node)>0:
        for node in torch_graph.nodes:
            if node.op=="output":
                args_new=()
                for output_node in node.args:
                    args_new=args_new+(output_node,)
                for extra_node in create_ouput_node:
                    args_new=args_new+(extra_node,)
                node.args=(args_new,)
    torch_graph.print_tabular()
    #torch_graph.lint()
    torch_model = fx.GraphModule(root=torch_modules, graph=torch_graph)
    return switch_paddings(torch_model)
