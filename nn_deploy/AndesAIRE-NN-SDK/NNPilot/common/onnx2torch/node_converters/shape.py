...
class OnnxShape(nn.Module, OnnxToTorchModuleWithCustomExport):  # pylint: disable=missing-class-docstring
    ...
    def forward(self, input_tensor: torch.Tensor) -> torch.Tensor:  # pylint: disable=missing-function-docstring
        if isinstance(input_tensor,Proxy):
            args = [input_tensor,]
            output = _ShapeExportToOnnx.set_output_and_apply(input_tensor, *args)
        else:
            output = torch.tensor(
...
def _(node: OnnxNode, graph: OnnxGraph) -> OperationConverterResult:  # pylint: disable=unused-argument
    #print()
    #print('~~~~~~~~~~~~~ I have got the op of shape ~~~~~~~~~~~~~')
    #print(node, node._input_values, node.attributes)
    #print(node.attributes.get('start', None))
    #print(node.attributes.get('end', None))
    #print()
