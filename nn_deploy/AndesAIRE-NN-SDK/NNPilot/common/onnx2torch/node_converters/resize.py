...
from onnx2torch.utils.common import onnx_mapping_from_node,get_const_value,get_shape_from_value_info
...
def _(node: OnnxNode, graph: OnnxGraph) -> OperationConverterResult:  # pylint: disable=unused-argument
    ...
    for input_value in node._input_values:
        if input_value in graph.initializers.keys():
            g=get_const_value(input_value,graph)
        else:
            input_value_info = graph.value_info[input_value]
    ...
    scale_factor=output_shape[-1]/input_shape[-1]
    size=tuple(output_shape[-2:])
    print(size)
    print(scale_factor)
    node._input_values = (node._input_values[0],)
    return OperationConverterResult(
            torch_module=torch.nn.Upsample(size=None, scale_factor=scale_factor, mode=mode, align_corners=None, recompute_scale_factor=None),
            onnx_mapping=onnx_mapping_from_node(node),
            )
    '''if len(node._input_values)==2:
        size_usp = graph.initializers[node._input_values[1]].to_torch().clone().detach().tolist()
        size_usp_0 = size_usp[0]
        size_usp_1 = size_usp[1]
        node._input_values = (node._input_values[0],)
        return OperationConverterResult(
            torch_module=torch.nn.Upsample(size=(size_usp_0,size_usp_1,), scale_factor=None, mode=mode, align_corners=None, recompute_scale_factor=None),
            #OnnxResize(
            #    mode=mode,
    ...
    elif len(node._input_values)==3:
        warnings.warn('torch.fx may not support the symbolic_trace of Upsampling with dynamic shape!!')
        scale_usp_1 = graph.initializers[node._input_values[2]].to_torch().clone().detach().tolist()
        node._input_values = (node._input_values[0],)
        return OperationConverterResult(
            torch_module=torch.nn.Upsample(size=None, scale_factor=scale_usp_1[-1], mode=mode, align_corners=None, recompute_scale_factor=None),
            onnx_mapping=onnx_mapping_from_node(node),
        )'''
