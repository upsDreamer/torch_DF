...
'''def _(node: OnnxNode, graph: OnnxGraph) -> OperationConverterResult:  # pylint: disable=unused-argument
    ...
    if len(node._input_values)==2:
        Pad_name = node._input_values[1]
        padding = graph.initializers[Pad_name]
        padding = padding.to_torch()
        padding = padding.tolist()
        print(padding)
        print(abc)
        pad_top, pad_bottom = padding[2], padding[6]
        pad_left, pad_right = padding[3], padding[7]
        zero_padding = nn.ZeroPad2d((pad_left,pad_right,pad_top,pad_bottom))
        return OperationConverterResult(
            torch_module=zero_padding,
            onnx_mapping=OnnxMapping(
        ...
        Pad_name = node._input_values[1]
        Value_name = node._input_values[2]
        padding = graph.initializers[Pad_name]
        ...
        pad_top, pad_bottom = padding[2], padding[6]
        pad_left, pad_right = padding[3], padding[7]
        constant_padding = nn.ConstantPad2d((pad_left,pad_right,pad_top,pad_bottom), value)
        return OperationConverterResult(
            torch_module=constant_padding,
            onnx_mapping=OnnxMapping(
                inputs=(node._input_values[0],),
                outputs=node.output_values,
            ),
