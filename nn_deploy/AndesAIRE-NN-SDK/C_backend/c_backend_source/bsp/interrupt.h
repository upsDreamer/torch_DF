...
#define HAL_INLINE_INTERRUPT_ATTACH(vector, isr)        { __vectors[vector] = (unsigned long)isr; }
#define HAL_INLINE_INTERRUPT_DETACH(vector, isr)        { if ( __vectors[vector] == (unsigned long)isr ) __vectors[vector] = (unsigned long)default_irq_entry; }

/*
 * Inline nested interrupt entry/exit macros
 */
/* Save/Restore macro */
#define SAVE_CSR(r)                                     unsigned long __##r = read_csr(r);
...
#ifdef __riscv_flen
#define SAVE_FCSR()                                     unsigned long __fcsr = read_fcsr();
